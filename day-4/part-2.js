...
const check_win = (board, unmarked_board) => {
  for (let i = 0; i < 4; i++) {
    if (
      ...
      console.log(`unscored_sum`, unscored_sum, `last_card`, last_card);
    ...
    for (let board = 0; board < boards.length; board++) {
      ...
      if (row !== undefined && col !== undefined) {
        ...
        if (win) {
          ...
          boardbools.splice(board, 1);
          boardstates.splice(board, 1);
          // console.log("removing".red, board, "with".red, card);
